...
class Assessment extends Base_Controller {
	...
	public function clearLocal() {
		...
		$this->load->view('assessment/clear_local_storage');
	}
	...
	{		
		...
		$this->data['users_list'] = $this->user_m->get_users();
		$this->data['class_list'] = $this->setup_m->get_class_list();
		$this->data['question_details'] = $this->assessment_m->get_questions_details_by_id2($this->uri->segment(3));
	...
	public function try_import () {
	    ...
	    redirect('assessment/view_questions/'.$this->input->post('question_id'));
	...
	{		
		$this->load->model('user_m');
		$this->data['title'] = 'View Correction';
		$user_id = $this->session->userdata('active_user')->id;
		if ($this->uri->segment(4)) {
			if($this->uri->segment(4) == $user_id) {
				////Check if uri-segment 3 is viewable
				$check_if_exam_viewable = $this->assessment_m->check_if_exam_viewable($this->uri->segment(3));
				if (isset($check_if_exam_viewable) && $check_if_exam_viewable->show_results ==1) {
					////Block access to other students' results
				$check_if_my_result = $this->assessment_m->check_if_my_result($this->uri->segment(3));
				if(count($check_if_my_result)>0) {
					$this->data['corrections'] = $this->assessment_m->get_assessment_grade_for_student2 ($this->uri->segment(3));

					$this->load->view('assessment/view_correction_students', $this->data);
				}
				else {
					show_404();
				}
	...
	{		
		...
		if ($this->session->userdata('active_user')->role_id == 3) {
			...
			///Get list of exams with viewable results
		$this->data['exam_list'] = $this->assessment_m->get_exams_with_viewable_results();
		//$this->data['assessment_grade'] = $this->assessment_m->get_assessment_grade_for_student2($this->uri->segment(3));
		$this->load->view('assessment/view_results_student', $this->data);
		}
		else {
	...
	if ($this->input->post('exam_type')=='1') {
		...
		$assessment_id = $this->input->post('assessment_id');
			...
			//$this->db->delete('exam_logs', $data);
	       	...
	       	$this->db->where('assessment_id', $this->input->post('assessment_idnum'));
		...
		$user_id = $this->session->userdata('active_user')->id;
		$question = $this->input->post('question');
		$correct = $this->input->post('myanswer');
		$assessment_id = $this->input->post('assessment_id');
		...
		foreach($question as $option_num => $option_val) {
			...
			$check_answer_row = $query_check_answer->row();
			$answerit = $check_answer_row->answer;
			if ($answerit == $correct[$option_num]) {
				$is_correct = 1;
			}
			else {
				$is_correct = 0;
	    ...
	    redirect('assessment/clearLocal');
